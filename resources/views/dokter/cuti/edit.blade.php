<x-admin.layout title="Edit Jadwal Cuti Dokter">
    {{-- Title Atas --}}
    <div class="mb-4 flex items-center justify-between">
        <h1 class="text-2xl md:text-4xl font-semibold">Edit Jadwal Cuti Dokter</h1>
        <a href="{{ route('admin.dokter.show', $dokter->id) }}"
            class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Kembali</a>
    </div>

    {{-- Form --}}
    <form action="{{ route('admin.dokter.cuti.update', $cuti->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        {{-- Nama Dokter --}}
        <div>
        ...
        <div>
            ...
            <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100" value="{{ $cuti->tanggal_mulai }}" required>
        </div>

        {{-- Tanggal Selesai --}}
        <div>
            <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Selesai</label>
            <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100" value="{{ $cuti->tanggal_selesai }}" required>
        </div>
